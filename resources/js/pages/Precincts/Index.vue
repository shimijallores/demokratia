...
function regenerateKeys(precinct: Precinct) {
    router.post(
        `/precincts/${precinct.id}/regenerate-keys`,
        {},
        {
            preserveScroll: true,
        },
...
<template>
            ...
            <CardContent>
                <div class="mb-4 flex gap-4">
                    ...
                    <Select
                        ...
                        @update:model-value="filterPrecincts"
                    >
                        ...
                        <SelectContent>
                            ...
                            <SelectItem value="transmitting"
                                >Transmitting</SelectItem
                            >
                ...
                <Table>
                    ...
                    <TableBody>
                        <TableRow
                            v-for="precinct in precincts.data"
                            :key="precinct.id"
                        >
                            ...
                            <TableCell class="px-4 py-4">
                                <Badge
                                    :class="statusBadgeClass(precinct.status)"
                                >
                                    {{ precinct.status }}
                                </Badge>
                            </TableCell>
                            <TableCell class="px-4 py-4 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <Button
                                        variant="outline"
                                        size="sm"
                                        @click="editingPrecinct = precinct"
                                    >
                                        Edit
                                    </Button>
                                    <Button
                                        variant="destructive"
                                        size="sm"
                                        @click="deletingPrecinct = precinct"
                                    >
                                        Delete
                                    </Button>
                                </div>
    ...
    <PrecinctEditModal
        :open="!!editingPrecinct"
        :precinct="editingPrecinct"
        @update:open="
            (val: boolean) => {
                if (!val) editingPrecinct = null;
            }
        "
    />
    <PrecinctDeleteModal
        :open="!!deletingPrecinct"
        :precinct="deletingPrecinct"
        @update:open="
            (val: boolean) => {
                if (!val) deletingPrecinct = null;
            }
        "
    />
    <PrecinctKeysModal
        :open="!!regeneratingPrecinct"
        :precinct="regeneratingPrecinct"
        @update:open="
            (val: boolean) => {
                if (!val) regeneratingPrecinct = null;
            }
        "
    />
</template>
